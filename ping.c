...
int main() {
    ...
    while (scanf("%c", &move) && player_1_score < 21 && player_2_score < 21) {  // Основной игровой цикл
        if (move != '\n') {  // Игнорирование нажатия Enter
            ...
            if ((ball_x == SIZE_X - 1) && (ball_vector_x == 1) &&
                ((ball_y == p2_racket) || (ball_y == p2_racket - 1) ||
                 (ball_y == p2_racket - 2))) {  // Проверка на отбивание мяча второй ракеткой
                ball_vector_x *= -1;  // Изменение направления движения мяча по горизонтали
            }
    ...
    // logic(command, &ball_x, &ball_y, &ball_vector_x, &ball_vector_y, &p1_racket, &p2_racket,
    // &player_1_score, &player_2_score, &restart);
...
int racket_move(int p1_racket, int p2_racket, char input_char) {
    ...
    if ((input_char == 'a') &&
        (p1_racket - 3) != 0) {  // Проверка на возможность движения первой ракетки вверх
    ...
    if ((input_char == 'k') &&
        (p2_racket - 3) != 0) {  // Проверка на возможность движения второй ракетки вверх
